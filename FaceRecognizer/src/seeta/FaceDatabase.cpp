...
namespace seeta
{
	namespace v2
	{
		...
		public:
			...
			{
				if (!features1 || !features2 || !similarity)
					return false;
			...
			static size_t SortAbove(IndexWithSimilarity *data, size_t N, float threshold)
			{
				...
				while (!sort_stack.empty())
				{
					...
					while (left < right)
					{
						while (left < right && data[right].similarity <= flag.similarity)
							--right;

						data[left] = data[right];

						while (left < right && data[left].similarity >= flag.similarity)
							++left;
				...
				{
					if (data[sorted_size].similarity < threshold)
						break;
				}
...
#define MAGIC_SERIAL 0x7726
		...
		FaceDatabase::FaceDatabase(const SeetaModelSetting &setting) : m_impl(new Implement(setting))
		{
		}
		...
		size_t FaceDatabase::QueryTopByCroppedFace(const SeetaImageData &cropped_face_image, size_t N, int64_t *index, float *similarity) const
		{
			if (!index || !similarity)
				return 0;
