...
namespace seeta
{
	...
	public:
		...
		Trans2D(T r1c1, T r1c2, T r1c3, T r2c1, T r2c2, T r2c3, T r3c1, T r3c2, T r3c3)
		{
			this->data(0) = r1c1;
			this->data(1) = r1c2;
			this->data(2) = r1c3;
	...
	public:
		...
		Vec3D() : x(this->data(0)), y(this->data(1)), z(this->data(2)) {}

		Vec3D(T x, T y, T z) : Vec3D()
		{
			this->data(0) = x;
			this->data(1) = y;
			this->data(2) = z;
	...
	public:
		...
		Vec2D() : x(this->data(0)), y(this->data(1)) {}
...
#undef __INNER_PRODUCT_Mn_p
	...
	namespace affine
	{
		...
		Trans2D<T> inverse(const Trans2D<T> &M)
		{
			...
			return Trans2D<T>(-M[4] * recip_t3t1_t0t4, M[1] * recip_t3t1_t0t4, -(M[1] * M[5] - M[4] * M[2]) * recip_t3t1_t0t4,
		...
		Trans2D<T> rotate(T degree)
		{
			...
			return Trans2D<T>(T(cos_degree), T(-sin_degree), T(0), T(sin_degree), T(cos_degree), T(0), T(0), T(0), T(1));
		}
		...
		{
			return Trans2D<T>(T(1), T(0), tx, T(0), T(1), ty, T(0), T(0), T(1));
		}
		...
		Trans2D<T> identity()
		{
			return Trans2D<T>(T(1), T(0), T(0), T(0), T(1), T(0), T(0), T(0), T(1));
		}
		...
		{
			return Trans2D<T>(sx, T(0), T(0), T(0), sy, T(0), T(0), T(0), T(1));
		}
		...
		{
			return Trans2D<T>(T(1), shx, T(0), shy, T(1), T(0), T(0), T(0), T(1));
		}
		...
		{
			return Trans2D<T>(fx ? T(-1) : T(1), T(0), T(0), T(0), fy ? T(-1) : T(1), T(0), T(0), T(0), T(1));
		}
