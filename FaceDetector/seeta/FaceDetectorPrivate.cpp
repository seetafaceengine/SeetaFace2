...
public:
	...
	{
		return ((image.channels == 3 || image.channels == 1) && image.width > 0 && image.height > 0 && image.data != nullptr);
	}
...
std::vector<Rect> Impl::SlidingWindow(const SeetaImageData &img, const SeetaImageData &img_pad, SeetaNet_Net *&net, float thres, int local_min_face, int local_max_face)
{
	...
	if (local_min_face <= 0)
		local_min_face = min_face_;

	float cur_scale = local_min_face / float(net_size);

	if (local_max_face <= 0)
		local_max_face = max_face_;
...
std::vector<Rect> Impl::RunNet(const SeetaImageData &img, SeetaNet_Net *&net, float thres, int dim, std::vector<Rect> &winList)
{
	...
	if (int(winList.size()) < local_max_batch_size)
		local_max_batch_size = int(winList.size());
...
static bool match_num1(const char *str, int i)
{
	...
	if ('0' <= ch && ch <= '9')
		return match_num1(str, i + 1);

	if (ch == 'k' || ch == 'K')
		return match_x(str, i + 1);

	if (ch == 'x')
		return match_pre_num2(str, i + 1);
	...
	: impl_(new Impl())
{
	...
	if (core_size.width > 0)
		p->width_limit_ = std::max<int>(100, core_size.width);

	if (core_size.height > 0)
		p->height_limit_ = std::max<int>(100, core_size.height);
...
SeetaFaceInfoArray FaceDetectorPrivate::Detect(const SeetaImageData &image)
{
	...
	if (local_max_face_size > 0)
		local_max_face_size = std::max(12, int(p->max_face_ * scale));
...
void FaceDetectorPrivate::GetScoreThresh(float *thresh1, float *thresh2, float *thresh3) const
{
	...
	if (thresh1 != nullptr)
		*thresh1 = p->class_threshold_[0];

	if (thresh2 != nullptr)
		*thresh2 = p->class_threshold_[1];

	if (thresh3 != nullptr)
		*thresh3 = p->class_threshold_[2];
}
