...
namespace seeta
{
	...
	public:
		...
		Image(int width, int height, int channels) : supper(height, width, channels)
		{
		}

		Image(const uint8_t *data, int width, int height, int channels) : supper(data, height, width, channels)
		{
		}
		...
		{
			if (blob.shape(0) != 1)
				throw  std::logic_error("Can not convert multi images.");
